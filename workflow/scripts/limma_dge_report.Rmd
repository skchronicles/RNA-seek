...
PCA colored by the following groups of interest: `r unique(sort(group))`. These represent the main variables of interest for the experiment.

```{r 3d-expression-pca-groups-after, out.width='100%', out.height='100%'}
...
cgroups <- as.factor(metadata$Group)
...
) %>% config(toImageButtonOptions = list(format = 'svg')) # png, svg, jpeg, webp

# Important: disable onclick() events
# fixes unexpected behavior with 
# multiple crosstalk plots
plotly::highlight(p, on = NULL)
```

The interactive 3D PCA plot above visualizes the clustering of samples based on the first three principal components (PCs) of the normalized gene expression data. Each point represents a sample, colored according to its experimental group. The axes indicate the percentage of variance explained by each PC, providing insights into the major sources of variation in the dataset.
...
Here is a table containing the differential expression comparisons to perform. Each row represents a comparison between two experimental groups.

```{r comparisons-table, results='asis'}
contrast_df <- contrastinfo
colnames(contrast_df) <- c("Experimental Group", "Baseline Group")
DT::datatable(
    contrast_df,
    extensions = c(
      "Buttons",       # add download buttons
...
deg_template <- '##### **{{grp1}} vs. {{grp2}}**

This section contains the results of the differential gene expression analysis between the `{{grp1}}` and `{{grp2}}` groups.
...
The table above contains the result of differential expression analysis using the R package `limma`. The table has been filtered to identify significant differentially expressed genes meeting the following thresholds: FDR adjusted p-value <= \`r fdr.threshold\` AND abs(FC) >= \`r fc.threshold\`. 
...
**Please note:** the full, unfiltered differential gene expression results can be found in the following TSV file: \`r paste("limma_voom_deg_", paste(ofh_grp1, paste(ofh_grp2, ".tsv", sep=""), sep = "-"), sep = "")\`.

##### **{{grp1}} vs. {{grp2}} Volcano Plot**
